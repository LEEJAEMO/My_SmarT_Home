...
| 구분 | 상태 | 사용자 작업 |
| --- | --- | --- |
| HAOS 가상 디스크 | 준비 완료 | 없음 |
| Galaxy Book AC 절전 해제 | 적용 완료 | 노트북을 전원에 연결해 운영 |
| VirtualBox·HA 가상머신 | 관리자 승인 대기 | 설치 스크립트 실행 및 UAC 허용 |
...
1. `http://homeassistant:8123`을 연다
2. VM 콘솔 또는 공유기 DHCP 목록에서 VM의 IP를 확인한 후 `http://IP주소:8123`을 연다
3. 아래 상태 점검 명령을 실행한다
...
`VirtualBoxInstalled: True`, `VmExists: True`, `VmState: running`, `HomeAssistantLocalPort: True`가 확인되면 정상이다. Home Assistant의 공식 문서도 `homeassistant.local` 또는 `:8123` 포트 접속을 안내한다.[^2]
...
| 증상 | 먼저 확인할 것 | 해결 |
| --- | --- | --- |
| `homeassistant.local`이 열리지 않음 | Galaxy Book 전원·VM 실행 상태 | 상태 점검 스크립트 실행 후 VM 재시작 |
