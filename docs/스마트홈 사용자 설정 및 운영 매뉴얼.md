...
| SwitchBot IR 리모컨 | Comfee 완료 / 나머지 진행 중 | 에어컨·전등·프로젝터 학습 및 Hub Mini 최종 배치 확인 |
...
최종 구성은 SwitchBot 앱의 **`Others` IR 리모컨**이다. `DIY Fan` 유형에서는 앱에 보이는 사용자 정의 버튼이 OpenAPI 호출에서 `3005 (invalid value)`로 거절될 수 있었고, `Others + customize` 방식으로 전환한 뒤 Home Assistant에서 실제 동작을 확인했다.

| 기능 | SwitchBot 버튼명 | HA 동작 의미 |
| --- | --- | --- |
| 전원 | `POWER` | 전원 토글 1회 |
| 풍량 | `Fan Speed 3` | 1 → 2 → 3 → 1 순환 |
| 타이머 | `Timer` | 1시간 → 2시간 → … → 6시간 순환 |
| 음소거 | `Mute` | 음소거 토글 |

- [x] `Others` 리모컨으로 등록
- [x] `POWER`, `Fan Speed 3`, `Timer`, `Mute` custom 명령 사용
- [x] Home Assistant의 `전원 / 풍량 변경 / 타이머 변경 / 음소거` 버튼에서 실제 동작 확인
- [x] 실제 `deviceId`는 `/config/secrets.yaml`의 `switchbot_fan_device_id`에만 저장하고 GitHub에는 기록하지 않음
- [ ] 물리 리모컨이나 SwitchBot 앱에서 별도 조작한 뒤에는 풍량/타이머 순환 위치가 달라질 수 있으므로 실제 기기 표시를 확인한다

선풍기는 현재 상태를 Home Assistant가 읽지 못하므로 `약풍`, `강풍`, `2시간` 같은 목표 상태 자동화를 만들지 않는다. 토글·순환 명령을 자동 재시도하지도 않는다.
...
- [ ] `냉방 26도로 켜`, `선풍기 풍량 바꿔`, `영화 모드`, `환기 시작`을 테스트한다
...
| 두 번 누르기 | 환기 | 에어컨 끄기, 환기 상태 기록. 선풍기는 필요하면 별도 버튼으로 조작 |
| 길게 누르기 | 수면 | 커튼 닫기 → 전등·프로젝터 끄기. 선풍기는 자동 목표 상태 제어하지 않음 |
...
| 외출 | S23가 집 밖에 10분 유지 | 에어컨·전등 끄기, 커튼 닫기. 선풍기는 자동 제어하지 않음 |
| 귀가 | S23가 집에 들어옴 | 낮 커튼 열기, 밤 전등 켜기, 온도 분기 |
| 영화 | `영화 모드` | 커튼 닫기 → 전등 끄기 → 2초 후 프로젝터 켜기 |
| 수면 | `수면 모드` | 커튼 닫기, 전등·프로젝터 끄기, 온도 조건부 에어컨 프리셋. 선풍기는 자동 제어하지 않음 |
| 환기 | 창문을 연 뒤 `환기 시작` | 에어컨 끄기, 환기 상태 기록. 선풍기는 필요하면 별도 버튼으로 조작 |

환기 장면은 창문과 선풍기를 자동으로 제어하지 않는다. CO₂가 800ppm 아래에서 10분 유지되면 환기 상태를 종료하고, 창문을 직접 닫으라는 S23 알림을 보낸다.
...
- [ ] 다이킨 제습 프리셋 5회 성공
- [ ] 다이킨 OFF 신호 5회 성공
- [x] Comfee 선풍기 전원·풍량 순환·타이머 순환·음소거 확인
...
- 물리 리모컨을 사용했다면 Home Assistant의 에어컨·프로젝터 추정 상태를 실제 상태에 맞춰 수동으로 고친다. 선풍기는 추정 상태를 저장하지 않으므로 풍량·타이머 순환 위치를 실제 기기에서 확인한다
...
| 선풍기 풍량/타이머 순서가 예상과 다름 | 물리 리모컨·SwitchBot 앱에서 별도 조작했는지 확인 | 실제 표시 상태를 확인하고 필요한 횟수만 수동으로 순환한다. 목표 상태 자동화는 사용하지 않는다 |
