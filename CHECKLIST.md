...
최종 확인된 구성은 `Fan` 유형이 아니라 **`Others` 유형 IR 리모컨 + `customize` 명령**입니다. 새 `Others` 리모컨에 실제로 동작하는 네 버튼만 만들고 이름을 정확히 아래처럼 유지합니다.

- `POWER`: 전원 토글
- `Fan Speed 3`: 풍량 1 → 2 → 3 → 1 순환
- `Timer`: 1시간 → 2시간 → … → 6시간 순환
- `Mute`: 음소거 토글

`setup/04_discover_switchbot_devices.ps1`에서 이 `Others` 리모컨의 `deviceId`를 확인해 `/config/secrets.yaml`의 `switchbot_fan_device_id`에만 저장합니다. 실제 ID는 GitHub에 커밋하지 않습니다.

Home Assistant에서는 `전원 / 풍량 변경 / 타이머 변경 / 음소거` 네 동작만 사용합니다. 선풍기의 실제 상태를 읽을 수 없으므로 약풍·강풍 같은 목표 상태를 자동으로 맞추거나 토글 명령을 재시도하지 않습니다.
...
- [x] Comfee 선풍기 `POWER` / `Fan Speed 3` / `Timer` / `Mute` 동작 확인
- [ ] 전등 ON/OFF/밝기와 프로젝터 정상 냉각 종료 시험
- [ ] `냉방 26도로 켜`, `선풍기 풍량 바꿔`, `영화 모드`, `환기 시작` 한국어 명령 시험
